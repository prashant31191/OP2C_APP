...
public class CallManager {
	...
	public void inviteAddress(LinphoneAddress lAddress, boolean videoEnabled, boolean lowBandwidth) throws LinphoneCoreException {
		...
		android.util.Log.w("yf25", "2. " + params.getVideoEnabled());
		if (videoEnabled && params.getVideoEnabled()) {
			params.setVideoEnabled(true);
			android.util.Log.w("yf25", "3. set true");
		} else {
			params.setVideoEnabled(false);
			android.util.Log.w("yf25", "4. set false");
		}
...
//			android.util.Log.w("yf25", "4. set false");
	...
	boolean reinviteWithVideo() {
		...
		android.util.Log.w("yf25", "Check if video possible regarding bandwidth limitations");
		bm().updateWithProfileSettings(lc, params);

		// Abort if not enough bandwidth...
		if (!params.getVideoEnabled()) {
			android.util.Log.w("yf25", "Abort if not enough bandwidth");
			return false;
		}
